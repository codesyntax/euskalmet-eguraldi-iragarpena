...
location_url = "euskalmet/weather/regions/basque_country/zones/{zone}/locations/{location}"
...
def download_locations(region='basque_country'):
    ...
    if response.ok:
        for item in response.json():
            location_responses = session.get(f'{BASE_URL}{zone_locations}'.format(
                region=region,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--download', action='store_true', help='Download the list of available locations and store in a JSON file')
    parser.add_argument('-l', '--locations', action='store_true', help='List all available locations')

    args = parser.parse_args()
    print('Download', args.download)
    print('Location', args.locations)
